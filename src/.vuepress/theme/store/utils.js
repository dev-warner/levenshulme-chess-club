...
export const navigation = state =>
  get(state, "blog.nav", []).map(nav => ({
    ...nav,
    active:
      state.route.path.split("/").join("") === nav.link.split("/").join(""),
    external: isExternal(nav.link)
  }));
...
export const posts = state => {
  ...
  return items
    ...
    .filter(
      item =>
        !tag(state) || ~item.tags.map(toLower).indexOf(toLower(tag(state)))
    )
    .sort((a, b) => a - b)
  ...
  switch (state.type) {
    case "category":
      return {
        ...
        description: `A collection of ${state.posts.length} ${
